...
async function save() {
  try { await window.storage.set('sim3-state', JSON.stringify(state), true); } catch(e) {}
}
async function load() {
  try {
    const r = await window.storage.get('sim3-state', true);
...
function makeChip(card, clickable, selected, onClick) {
  ...
  chip.className = 'card-chip' + (isRed(card) ? ' red' : '') + (clickable ? ' clickable' : '') + (selected ? ' sel' : '');
...
async function doHostLogin() {
  ...
  if (pin === HOST_PIN) { isHost = true; showScreen('setup-screen'); startPoll(); }
  else document.getElementById('host-login-error').textContent = 'Incorrect PIN.';
}
...
function doLogout() { currentTeam = null; isHost = false; stopPoll(); showScreen('login-screen'); }
...
function finalizeSetup() {
  if (!Object.keys(state.teams).length) { document.getElementById('setup-msg').textContent = 'Add at least one team first.'; return; }
...
async function submitTrade() {
  ...
  if (myCards === 0 && theirCards === 0) { setTradeMsg('Select at least one card to include.', true); return; }
  if (myCash > 0 && theirCash > 0) { setTradeMsg('Only one side can offer cash.', true); return; }
  if (myCards > 0 && theirCards > 0 && myCash === 0 && theirCash === 0) { setTradeMsg('Card-for-card trades are not allowed. One side must offer cash.', true); return; }
  if (myCards === 0 && theirCards === 0 && (myCash > 0 || theirCash > 0)) { setTradeMsg('Cash-for-cash trades are not allowed. One side must offer cards.', true); return; }

  await load();
  const myTeam = state.teams[currentTeam];
  if (myCash > myTeam.cash) { setTradeMsg("You don't have enough cash.", true); return; }
  for (const c of mySelCards) { if (!myTeam.cards.includes(c)) { setTradeMsg(`You no longer hold ${c}.`, true); return; } }
...
async function respondTrade(tradeId, accept) {
  ...
  if (accept) {
    const from = state.teams[trade.from]; const to = state.teams[trade.to];
    if (!from || !to) return;
    ...
    for (const c of trade.fromCards) { if (!from.cards.includes(c)) { alert(`${trade.from} no longer holds ${c}.`); return; } }
    for (const c of trade.toCards) { if (!to.cards.includes(c)) { alert(`You no longer hold ${c}.`); return; } }
...
function refreshHostDashboard() {
  ...
  Object.entries(state.teams).forEach(([name, t]) => {
    ...
    card.innerHTML = `<p style="font-weight:500; margin-bottom:6px;">${name}</p>
      ...
      <p style="font-size:13px; margin-bottom:6px;">Trades: <strong>${t.trades||0}</strong> <span class="badge ${ok?'badge-accepted':'badge-pending'}">${ok?'qualified':(MIN_TRADES-(t.trades||0))+' more needed'}</span>${pending?` <span class="badge badge-pending">${pending} pending</span>`:''}</p>`;
  ...
  if (!done.length) {
    const p = document.createElement('p'); p.style.cssText = 'font-size:13px; color:var(--color-text-secondary);';
...
function buildResults() {
  ...
  rankTable.appendChild(rg);
  content.appendChild(rankTable);
...
(async () => { await load(); populateLoginTeams(); })();
